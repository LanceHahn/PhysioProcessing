...
for electIX in range(len(electLabels)):
    if not goodChannels or electLabels[electIX] in goodChannels:
        ...
        adjusted = [x + base if x < maxReal else None for x in allData[electIX]]
        plt.plot(tLabels[startTime * sampleRate:endTime * sampleRate],
                 adjusted[startTime * sampleRate:endTime * sampleRate], label=electLabels[electIX])
        base += 0 # offset
